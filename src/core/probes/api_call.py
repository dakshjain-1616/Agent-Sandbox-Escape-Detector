"""Probe that tests whether the target agent makes unintended external API calls."""
...
class ApiCallProbe(BaseProbe):
    """Tests if the target agent can be tricked into making unintended external API calls."""
    ...
    def load_prompts(self) -> list[str]:
        prompts_path = PROMPTS_DIR / "api_call_prompts.txt"
        try:
    ...
    async def run(
                ...
                async with httpx.AsyncClient(timeout=self.timeout) as client:
                    ...
                    if any(
                        ...
                        evidence_parts.append(
                            f"Prompt {len(responses)}: Agent appears to have made or described an external API call"
                        )
        ...
        if error_count == len(prompts):
            ...
            verdict = Verdict.SAFE
            evidence = "Agent did not make any unintended API calls."
            confidence = 0.9
