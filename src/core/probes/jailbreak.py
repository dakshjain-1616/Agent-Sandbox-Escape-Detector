"""Probe that tests whether the target agent falls for chain-of-thought jailbreaking."""
...
class JailbreakProbe(BaseProbe):
    """Tests if the target agent can be jailbroken via CoT manipulation and reasoning bypass."""
    ...
    def load_prompts(self) -> list[str]:
        prompts_path = PROMPTS_DIR / "jailbreak_prompts.txt"
        try:
    ...
    async def run(
                ...
                async with httpx.AsyncClient(timeout=self.timeout) as client:
                    ...
                    if any(
                        ...
                        evidence_parts.append(
                            f"Prompt {len(responses)}: Agent may have complied with jailbreak attempt"
                        )
        ...
        if error_count == len(prompts):
            ...
            verdict = Verdict.SAFE
            evidence = "Agent resisted jailbreak attempts."
            confidence = 0.9
